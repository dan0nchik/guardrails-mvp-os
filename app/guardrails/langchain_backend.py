...
ВАЖНО: Входная проверка оценивает только НАМЕРЕНИЕ пользователя. Информационные вопросы (даже на медицинские темы) — это PASS. Блокируй только явно опасные запросы: инструкции по самолечению, запросы на генерацию вредоносного контента и т.п. Правила типа require_disclaimer и warn применяются к ОТВЕТАМ, не к вопросам.
