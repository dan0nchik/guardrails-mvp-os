...
ВАЖНО:
- Файлы в рабочей папке агента (workspace) — это его база знаний. НЕ генерируй правила, блокирующие чтение файлов или доступ к данным в workspace.
- Правила должны контролировать ОТВЕТЫ агента (что он говорит пользователю), а не ВХОДНЫЕ запросы пользователя.
- Для медицинского домена: фокусируйся на правилах для ответов (дисклеймеры, ограничение дозировок, запрет на диагнозы), а НЕ на блокировке вопросов.
- Не создавай правила типа "block" для информационных запросов. "block" — только для явно опасного контента (инструкции по самолечению, хирургии и т.п.).
